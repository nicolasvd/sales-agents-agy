...
Write the full output to `reports/{slug}/markdown/OUTREACH-SEQUENCE.md`:
...
*Generated by AI Sales Team — Cold Outreach Engine*
```
...
- If `reports/{slug}/markdown/COMPANY-RESEARCH.md` exists, use company data for personalization and context
- If `reports/{slug}/markdown/DECISION-MAKERS.md` exists, use contact profiles and personalization anchors
- If `reports/{slug}/markdown/LEAD-QUALIFICATION.md` exists, use pain points and buying signals for messaging
- If `reports/{slug}/markdown/COMPETITIVE-INTEL.md` exists, use competitive positioning for differentiation angles
- Suggest follow-up: `prep <url>` for meeting preparation after getting a response, `followup <url>` for post-meeting sequence, `objections <url>` for deeper objection handling
