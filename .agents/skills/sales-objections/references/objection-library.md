...
Based on the `<topic/industry>` provided, generate 5 additional objections unique to that industry. Use the same format as the universal objections above (real meaning, FFR response, A-R-C response, follow-up question, proof point, when to walk away).
...
If the user provides a URL, use read_url_content to research the specific company and generate objections tailored to that company's situation, not just the industry.
